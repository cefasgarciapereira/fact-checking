...
class FaceBot:
    ...
    def post_info(self, df='', append_df=False, is_valid=True):
        ...
        if(append_df == True):
            # adiciona informações do poynter
            info = {
                "id" : df['id'],
                "checked_link": df['checked_link'],
                ...
                "source": df['source'],
                "title": df['title'],
                "poynter_link": df['poynter_link'],                   

                "facebook_id": self.id,
                "author": self.rc(author),
                "facebook_date": self.rc(date),
